...
@AFK.on_message(
    (filters.chat(Config.GROUPS) | filters.chat(Config.AUTH_USERS)) &
    filters.incoming & filters.command("down", prefixes=prefixes)
)
async def down(bot: AFK, m: Message):
...
@AFK.on_message(
    (filters.chat(Config.GROUPS) | filters.chat(Config.AUTH_USERS)) &
    filters.incoming & filters.command("cpd", prefixes=prefixes)
)
async def cpd(bot: AFK, m: Message):
    editable = await m.reply_text(
            "Hello Bruh **I am Text Downloader Bot**. I can download videos from **text** file one by one.**\n\nDeveloper** : उससे क्या होगा काम से काम रखो 😂 @DONTCRY4U**\nLanguage** : Python**\nFramework** : Pyrogram\n\nSend **TXT** File {Name : Link}"
       ,reply_markup=keyboard)
    input: Message = await bot.listen(editable.chat.id)
...
@AFK.on_message(
    (filters.chat(Config.GROUPS) | filters.chat(Config.AUTH_USERS)) &
    filters.incoming & filters.command("dhurina", prefixes=prefixes)
)
async def dhurina(bot: AFK, m: Message):
...
@AFK.on_message(
    (filters.chat(Config.GROUPS) | filters.chat(Config.AUTH_USERS)) &
    filters.incoming & filters.command("vision", prefixes=prefixes)
)
async def vision(bot: AFK, m: Message):
