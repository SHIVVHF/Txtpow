...
class Msg(object):
    START_MSG = "**Running1 !!**"
    START_MSG2 = "<b>Hey welcome bro  type command:-"\
        "\n\n`Hi BOSS I'm Alive Send download all `/pro `"\
    "\n`for classplus send `/cpd` for dhurina send  `/dhurina`\nfor vision send `/vision`"\
    "\n`for Text send `/pro_txt`\nfor Top send  `/pro_top`\nfor vision send `/pro_vision`\nfor Jw send `/pro_jw`\nfor Olive send `/pro_olive`\nfor Addapdf send `/adda_pdf`"\
    "\n\n`extra cource vip for allpdf+html:- \n`/pro` and `/cpdbypj`"\
         "\n\n`for task close `/cancel `"\
